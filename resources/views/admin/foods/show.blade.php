...
@php
    // Helper functions defined inline (temporary fix)
    if (!function_exists('proteinCalories')) {
        function proteinCalories($grams) { return $grams * 4; }
    }
    if (!function_exists('carbsCalories')) {
        function carbsCalories($grams) { return $grams * 4; }
    }
    if (!function_exists('fatCalories')) {
        function fatCalories($grams) { return $grams * 9; }
    }
    if (!function_exists('caloriesDailyValue')) {
        function caloriesDailyValue($calories) { return round(($calories / 2000) * 100); }
    }
    if (!function_exists('proteinDailyValue')) {
        function proteinDailyValue($grams) { return round(($grams / 50) * 100); }
    }
    if (!function_exists('carbsDailyValue')) {
        function carbsDailyValue($grams) { return round(($grams / 275) * 100); }
    }
    if (!function_exists('fatDailyValue')) {
        function fatDailyValue($grams) { return round(($grams / 78) * 100); }
    }
    if (!function_exists('fiberDailyValue')) {
        function fiberDailyValue($grams) { return round(($grams / 28) * 100); }
    }
